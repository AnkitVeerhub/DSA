...
Tests stack usage understanding.

Evaluates ability to handle reverse operations.

Real-world trace and simulation problems are common in systems.
...
Microsoft
'''
def track_final_ball_possession(A,B,C):
    stack = [B]
    for move in C:
        if move == 0:
            if len(stack) > 1:
            ...
            stack.append(move)
    return stack[-1]
print(track_final_ball_possession(A = 5, B = 23, C = [1, 2, 0, 1, 0]))
